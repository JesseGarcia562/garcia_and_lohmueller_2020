...
ceu_df<-as_tibble(read_rds(file = "ceu_physical_breaks_200.rds")) %>%
  dplyr::rename(d=X6,variation=variation_type, r_squre=genetic_distance, genetic_distance=X3) 
chb_df<-as_tibble(read_rds(file = "chb_physical_breaks_200.rds")) %>%
  dplyr::rename(d=X6,variation=variation_type, r_squre=genetic_distance, genetic_distance=X3) 


combine_df<- bind_rows(empiricalDf %>% mutate(type="yri") %>% select(-chromosome), ceu_df %>% mutate(type="ceu"), chb_df %>% mutate(type="chb") ) %>%  mutate(variation=case_when(variation=="synonymous_SNV" ~ "Synonymous", variation=="nonsynonymous_SNV" ~ "Nonsynonymous" , TRUE ~ variation   )) %>%
  mutate(allele_count=coalesce(allele_count, as.integer(ac)))


cols <- c("Simulated" = "#E69F00", 
          "Empirical (YRI)" = "#009E73", 
          "Empirical (CHB)" = "#56B4E9",
          "Empirical (CEU)" =  "#F0E442",
          "Resample mean" ="#E69F00", 
          "Neutral" ="#999999")


lines <- c("Simulated" = "solid", 
           "Empirical (YRI)" = "solid", 
            "Empirical (CHB)" = "solid",
            "Empirical (CEU)" = "solid",
           "Resample mean" ="solid", 
...
ceu_yri_breaks<-bin_one_population_into_another_compute_mean_difference(ld_df = ceu_df, empiricalDf = empiricalDf, population_of_ld_df="CEU")  %>% 
  mutate(resample=99993)

chb_yri_breaks<-bin_one_population_into_another_compute_mean_difference(ld_df = chb_df, empiricalDf = empiricalDf, population_of_ld_df="CHB") %>% 
  mutate(resample=99994)


si_fig_15 <- bind_rows(ceu_yri_breaks, chb_yri_breaks, graphDf) %>%
  ...
  scale_color_manual(values=cols, breaks=c("Neutral", "Empirical (CEU)", "Empirical (YRI)","Empirical (CHB)","Simulated")) +
